...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ project?: string }> }) {
  ...
  let query = supabase
    .from("customers")
    .select(`id, full_name, email, phone, tags, last_order_date, lifetime_orders, lifetime_spent, project:project_id(name), orders(id, order_number, created_at, products_summary, total_price, currency, status)`)
    .is("deleted_at", null)
