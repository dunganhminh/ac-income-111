...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<{ project?: string }> }) {
  ...
  let query = supabase
    .from("orders")
    .select("id, project_id, order_number, created_at, status, total_price, currency, tags, payment_method, gateway_transaction_id, shipping_fee, paypal_fee, total_income, products_summary, customer:customer_id(full_name, email, phone)")
    .is("deleted_at", null)
